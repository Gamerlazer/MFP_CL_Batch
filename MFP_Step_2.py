...
sfdc = 'SFDC_Report_modified_2016-02.csv'
atr_report = 'MFP - Admin Query - 022016.csv'
ones_got_away = 'Ones that got away_'+lm+'.csv'
...
with open(atr_report,'rU') as r1, open(sfdc,'rU') as r2, open(cleaned_cc_trans_report,'r') as r3, open(recon_report,'w') as w1:
	...
	atr_list = {row['business_id'] : row['advertiser_id'] for row in atr_r}
	...
	for row in sfdc_r:
		biz_id = row[biz_id_i] 
		if biz_id in atr_list and atr_list[row[biz_id_i]] in cc_trans_list:
